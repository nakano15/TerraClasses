...
namespace TerraClasses.SkillList.Vilemancer
{
    public class CorruptQuills : SkillBase
    {
        ...
        public override void Update(Player player, SkillData rawdata)
        {
            ...
            if(data.Time >= 70)
            {
                int QuillNumber = (int)((data.Time - 70) * 0.1f);
                if ((data.Time - 70) % 10 == 0)
                {
                    int Target = QuillNumber % data.Targets.Length;
                    if(!data.Targets[Target].IsDead())
        ...
        public override void Draw(Player player, SkillData rawdata, PlayerDrawInfo pdi)
        {
            ...
            for(int q = 0; q < QuillCount; q++)
            {
                ...
                float TargetDirection = (float)Math.Atan2(QuillPosition.Y - Target.Center.Y, QuillPosition.X - Target.Center.X) + MathHelper.ToRadians(90);
                if(QuillTime >= 60)
                {
                    QuillPosition += (Target.Center - QuillPosition) * ((QuillTime - 60) * 0.1f);
                }
                Terraria.DataStructures.DrawData dd = new Terraria.DataStructures.DrawData(QuillTexture, QuillPosition - Main.screenPosition, null, Color.White, TargetDirection, new Vector2(3, 8), 1f, SpriteEffects.None, 0 );
